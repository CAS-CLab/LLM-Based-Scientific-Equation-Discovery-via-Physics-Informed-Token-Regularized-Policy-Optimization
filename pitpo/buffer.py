...
class ExperienceBuffer:
    ...
    def reset_islands(self) -> None:
        ...
        # Sort best scores without adding random noise.
        ...
        for island_id in reset_islands_ids:
            ...
            founder_island_id = np.random.choice(keep_islands_ids) if len(keep_islands_ids) > 0 else None
            founder = (self._best_program_per_island[founder_island_id]
                       if founder_island_id is not None else None)
            founder_scores = (self._best_scores_per_test_per_island[founder_island_id]
                              if founder_island_id is not None else None)
            # Only register a founder if one exists (e.g., after initial seeding)
...
class Island:
    ...
    def get_prompt(self) -> tuple[str, int]:
        ...
        # If the island has not been seeded yet, bootstrap with the template's base function
        if len(self._clusters) == 0:
            try:
                base_impl = copy.deepcopy(self._template.get_function(self._function_to_evolve))
            except Exception as e:
                # Surface a clearer error to aid debugging
                raise RuntimeError(f"Failed to locate base function '{self._function_to_evolve}' in template: {e}")
            # Generate prompt with a single baseline implementation (v0) and empty header for v1
            implementations = [base_impl]
            version_generated = len(implementations) + 1  # next version after provided ones
            return self._generate_prompt(implementations), version_generated
